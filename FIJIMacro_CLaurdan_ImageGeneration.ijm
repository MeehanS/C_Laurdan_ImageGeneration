/* ImageJ macro for GP images analysis
   ...
   The original macro has been minimally modified by Sean Meehan [University of Miami] and mods marked as SM
   - Changes address GP calculation issues with original script
   Current version July 2022, GitHub: MeehanS
*/
...
for (h = 0, j = h; h < listDisord.length; h++, j++) {
	...
	run("Duplicate...","title=Image_Add_forMask.tif"); // SM
	...
	selectImage("Image_Add_forMask.tif");
	setThreshold(t*2,510);
	run("Convert to Mask");
	run("Duplicate...", "title=Image_Mask.tif"); //SM
	run("Invert"); // SM
	run("Create Selection"); // SM
	selectImage("Image_preGP.tif"); 
	run("Restore Selection"); // SM
	run("Set...", "value=NaN"); // SM
	run("Select None"); // SM
	run(lut1);
